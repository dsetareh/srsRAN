...
if [ $# -ne 2 ] && [ $# -ne 3 ] && [ $# -ne 4 ]
  then
    echo -e "Syntax: ./startFuzzing.sh <test start index> <test end index> <-d> (Optional, to delete existing log files) <-i> iteration"
    exit 1
fi
...
do
    ...
    do
        ...
        /home/srsran/srsRAN/build/srsepc/src/srsepc >> fuzzLogs/epcFuzzLogs.txt &
        epc_pid=$!

        # start enb, append output to log file, store pid, echo pid
        /home/srsran/srsRAN/build/srsenb/src/srsenb --rf.device_name=zmq --rf.device_args="fail_on_disconnect=true,tx_port=tcp://*:2000,rx_port=tcp://localhost:2001,id=enb,base_srate=23.04e6" >> fuzzLogs/enbFuzzLogs.txt  &
        enb_pid=$!
        sleep 1 # * 1 second wait

        # start ue, append output to log file, store pid, echo pid
        /home/srsran/srsRAN/build/srsue/src/srsue -f$i >> fuzzLogs/ueFuzzLogs.txt &
        ue_pid=$!
        ...
        sleep 2 # wait a bit for enb to save pcap before moving
        echo -e "${NC}"

        # save pcap file for enb
        
        echo -e "\n${GREEN}Saving ENB PCAP as fuzzLogs/pcap/$i/$(date +"%T").pcap${NC}\n"
        cp enb.pcap fuzzLogs/pcap/$i/$(date +"%T").pcap
